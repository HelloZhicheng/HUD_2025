---
title: "PCA Adaptivity Recovery Analysis"
---
...
Eco[,c(7,10,13)]<- -Eco[,c(7,10,13)]
Health[,c(9,10)]<- -Health[,c(9,10)]
```
...
all_data <- cbind(Social[,-c(1:5)],Health[,-c(1:5)],Eco[,-c(1:5)],Infra[,-c(1:5)])
...
raw_loadings <- pca_all$rotation[, 1:7] %*% diag(pca_all$sdev, 7, 7)
...
unweighted_scores <- as.matrix(scaled_data) %*% pca_all$rotation[, 1:7]
...
weighted_scores <- unweighted_scores %*% variance_exp$proportion_variance[1:7]
...
write.csv(index_result, "Recovery_index.csv")
