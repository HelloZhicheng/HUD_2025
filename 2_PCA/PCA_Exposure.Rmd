...
Meta <- raw_data$Metadata[-c(263:268), ]

# Transform specific variables to negative (for standardization purposes)
Env$DEM <- -Env$DEM
Env$ksat <- -Env$ksat
Env$change_forestR <- -Env$change_forestR
Infra$dis_water <- -Infra$dis_water
Infra$road_1000 <- -Infra$road_1000
```
...
all_data <- cbind(Social[, -c(1:5)], Env[, -c(1:5)], Infra[, -c(1:5)])
...
weighted_scores <- as.matrix(all_data_z) %*% as.matrix(loadings) %*% variance_exp$proportion_variance[1:6]
...
raw_loadings <- loadings %*% diag(pca_result$sdev, 6, 6)
...
```{r}
# Export results
write.csv(data.frame(score = unweighted_scores, rank = unweighted_rank), "Exposure.csv")
write.csv(data.frame(score = weighted_crm, rank = weighted_rank_crm), "Exposure_crm.csv")
```
